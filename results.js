...
// Append new item cleanly inside the document layout mapping structure
async function addNewSubjectToFirebase(subjectName) {
    try {
        const snapshot = await fetchCurrentState();
        if (!(subjectName in snapshot)) {
...
// Sync interface cleanly with live document snapshots
onSnapshot(docRef, (docSnap) => {
    if (docSnap.exists()) {
        renderSubjectsTable(docSnap.data());
    } else {
